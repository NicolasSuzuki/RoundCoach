...
export function simulateFakeAnalysis(
  ...
  const crosshairScore = clampScore(
    crosshairBase +
      agentProfile.crosshair +
      mapProfile.crosshair +
      resultProfile.crosshair,
  );
  const utilityUsageScore = clampScore(
    utilityBase +
...
function getResultProfile(result: MatchResult, delta: number) {
  const clampedDelta = Math.max(-10, Math.min(10, delta));
  const winBias = result === MatchResult.WIN ? 1 : -1;
...
function parseScore(score: string, result: MatchResult) {
  ...
  if (!matched) {
    return { own: result === MatchResult.WIN ? 13 : 9, enemy: result === MatchResult.WIN ? 9 : 13, delta: result === MatchResult.WIN ? 4 : -4 };
  }

  const left = Number(matched[1]);
  const right = Number(matched[2]);
  const own = result === MatchResult.WIN ? Math.max(left, right) : Math.min(left, right);
  const enemy = result === MatchResult.WIN ? Math.min(left, right) : Math.max(left, right);
